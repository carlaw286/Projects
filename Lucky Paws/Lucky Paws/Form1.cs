...
namespace Lucky_Paws
{
    public partial class Form1 : Form
    {
        ...
        string connectionStringAdopt = "Data Source=DESKTOP-H2JR9D8;Initial Catalog=Adopter_DB;Integrated Security=True";
        public Form1()
        {
            InitializeComponent();
        }
        ...
        void Clear()
        {
            ...
            ValidID_TextBox.Text = PP_TextBox.Text = "";
            // Look for shelter
        ...
        private void Button_LogIn_Click(object sender, EventArgs e)
        {
            ...
            command.CommandText = "SELECT * FROM CreateAccount_Table Where Username = @Username";
            ...
            string query = "Select * from CreateAccount_Table Where Username = '" + Username_TextBox.Text.Trim()
            ...
            if (dtbl.Rows.Count == 1)
            {
                ...
                sqlCon.Open();
                SqlCommand cmd = new SqlCommand("Select Username, FirstName, LastName, BMonth, BDay, BYear, Gender, CompleteAddress, MobileNumber, EmailAddress, Verification from CreateAccount_Table where Username =@Username", sqlCon);
                ...
                while (da.Read())
                {
                    ...
                    PP_Verification.Text = da.GetValue(10).ToString();
                }
                Clear();
                sqlCon.Close();
            }
        ...
        private void UploadValidID_Button_Click(object sender, EventArgs e)
        {
            ...
            OD.Filter = "Supported Images|*.jpg;*.jpeg;*.png";
            if (OD.ShowDialog() == DialogResult.OK)
                CreateAcc_ValidID.Load(OD.FileName);
        ...
        private void SignUp_Button_Click(object sender, EventArgs e)
        {
            ...
            else if (ValidID_TextBox.Text == "" || PP_TextBox.Text == "" || CreateAcc_Password.Text == "" || CreateAcc_Username.Text == "")
            {
            ...
            else
            {
                ...
                var image13 = new ImageConverter().ConvertTo(CreateAcc_ValidID.Image, typeof(Byte[]));
                ...
                command.Parameters.AddWithValue("@Verification", pending);
                command.Parameters.AddWithValue("@Valid_ID", image13);
                command.CommandText = "INSERT INTO CreateAccount_Table (ProfilePic,Username,Password,FirstName,LastName,BMonth,BDay,BYear,Gender,CompleteAddress,MobileNumber,EmailAddress,Verification,Valid_ID) VALUES (@ProfilePic,@Username,@Password,@FirstName,@LastName,@BMonth,@BDay,@BYear,@Gender,@CompleteAddress,@MobileNumber,@EmailAddress,@Verification,@Valid_ID)";
        ...
        bool checkUsername()
        {
            ...
            string query = "Select * from CreateAccount_Table Where Username =@Username";
        ...
        private void LFSPost_Button_Click(object sender, EventArgs e)
        {
                ...
                command.CommandText = "INSERT INTO LookForShelter_Table (Pet_Photo,Username,Name,Sex,Age,Animal_Type,Color,Address,Description,Email,Contact_Number,PP) VALUES (@Pet_Photo,@Username,@Name,@Sex,@Age,@Animal_Type,@Color,@Address,@Description,@Email,@Contact_Number,@PP)";
                ...
                con.Open();
                command.CommandText = "SELECT * FROM LookForShelter_Table Where Name = @Name";
                SqlDataReader reader = command.ExecuteReader();
                if (reader.Read())
                {
                    MemoryStream stream = new MemoryStream(reader.GetSqlBytes(1).Buffer);
        ...
        {
            CheckStatus_Panel.Visible = PetPicked_Panel.Visible = PF_Content1_Panel.Visible = false;
        ...
        private void PF_Content1_Panel_Paint(object sender, PaintEventArgs e)
        {
            ...
            {
                CheckStatus_Button.Visible = true;
                Adopt_Button.Visible = false;
            }
            else
            {
                Adopt_Button.Visible = true;
                CheckStatus_Button.Visible = false;
            }
        }
        ...
        private void Adopt_Button_Click(object sender, EventArgs e)
        {
            ...
            SqlConnection con = new SqlConnection(connectionStringAdopt);
            con.Open();
            SqlCommand command = con.CreateCommand();
            var image = new ImageConverter().ConvertTo(Prof_pic1.Image, typeof(Byte[]));
            command.Parameters.AddWithValue("@PP", image);
            command.Parameters.AddWithValue("@Username", PP_Username.Text);
            command.Parameters.AddWithValue("@FullName", PP_FullName.Text);
            command.Parameters.AddWithValue("@Email", PP_Email.Text);
            command.Parameters.AddWithValue("@ContactNumber", PP_ContactNumber.Text);
            command.Parameters.AddWithValue("@Verification", PP_Verification.Text);

            command.CommandText = "INSERT INTO Adopter_Table (PP,Username,FullName,Email,ContactNumber,Verification) VALUES (@PP,@Username,@FullName,@Email,@ContactNumber,@Verification)";
            command.ExecuteNonQuery();
            MessageBox.Show("Pet Sucessfully Picked!");
            con.Close();
            ...
            con.Open();
            command.CommandText = "SELECT * FROM Adopter_Table Where Username = @Username";
            SqlDataReader reader = command.ExecuteReader();
            if (reader.Read())
            {
                MemoryStream stream = new MemoryStream(reader.GetSqlBytes(1).Buffer);
                Adopter_PP.Image = Image.FromStream(stream);
                Adopter_Username.Text = reader.GetValue(2).ToString();
                Adopter_FullName.Text = reader.GetValue(3).ToString();
                Adopter_Email.Text = reader.GetValue(4).ToString();
                Adopter_ContactNumber.Text = reader.GetValue(5).ToString();
                Adopter_Status.Text = reader.GetValue(6).ToString();
        ...
        private void CheckStatus_Panel_Paint(object sender, PaintEventArgs e)
        {
            if (c == 1)
            {
                Adopter1_Panel.Visible = true;
                Adopter1_Border.Visible = true;
            }
        }

        private void CheckStatus_Button_Click(object sender, EventArgs e)
        {
            CheckStatus_Panel.Location = new System.Drawing.Point(28, 50);
            CheckStatus_Panel.Visible = true;
            CheckStatus_Panel.Show();
            CheckStatus_Panel.BringToFront();
        }
    }
}
